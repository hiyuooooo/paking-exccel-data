...
class TransactionManager {
  ...
  addTransaction() {
    ...
    const transaction = {
      ...
      details:
        document.getElementById("details").value || "No details provided",
    };
    ...
    this.showNotification(
      `Transaction added successfully! ${transaction.type === "deposit" ? "+" : ""}₹${Math.abs(transaction.amount)}`,
    );
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    app.showNotification(
      'Welcome to Transaction Manager! Press "A" to quickly add a transaction.',
    );
  }, 1000);
});
